...
class Organism:
    def __init__(self, optimum, n_organisms, mu_mean, sigma_covariance, n_dim, mutation_rate, gen_duplication_rate,
        ...
        self.gen_mode = gen_mode  # Determinates if we continue the evaluation until the optimum is reached or if we continue until the number of generations is met
    ...
    def mutation(self, genotype):
        ...
        if random.random() <= self.mutation_rate:
            ...
            #     genotype = genotype - vector_substract_value
                ...
                # genotype[gen_position] = mutated_gen[0:]
                ...
                #    np.shape(vector_substract_value)) + ' Mutated GEN = ' + str(np.shape(mutated_gen)) + ' Mutated genotype : ' + str(np.shape(genotype)))
    ...
    def run(self):
        ...
        if self.gen_mode:
            ...
            while father_organism == self.optimum or self.epsilon <= fitness_value:
                ...
                selected_genotype = self.selection(son_genotype, father_genotype)
                # selected_organism = self.create_organism(selected_genotype)
                print('Best suited organism is : ' + str(np.add.reduce(selected_genotype)))
                print('Best suited genotype is : ' + str(selected_genotype))
